...
ciclos<-length(DTC)
DTC<-sum(DTC)
NPcp<-nrow(subset(df,df$FECHA.DESTETE>Fi&df$FECHA.DESTETE<Ff))
...
a. CON período recría – pubertad y CON baja
```{r}
ciclos<-length(Dur.Total)
Dias.improd.ciclo<-((sum(Dur.Total)-sum(DG)-sum(DL))/ciclos)
Porc.Dias.improd.ciclo<-(Dias.improd.ciclo*100)/mean(Dur.Total)
...
DTC<-c(Ci,Cb)
ciclos<-length(DTC)
Dias.improd.ciclo.Pub.Bja<-(sum(DTC)/ciclos)
Porc.Dias.improd.ciclo.Pub.Bja<-(Dias.improd.ciclo.Pub.Bja*100)/mean(Dur.Total)
...
DTC<-c(Ci)
ciclos<-length(DTC)
Dias.improd.ciclo.Pub<-(sum(DTC)/ciclos)
Porc.Dias.improd.ciclo.Pub<-(Dias.improd.ciclo.Pub*100)/mean(Dur.Total)
Porc.Dias.improd.ciclo.Pub
...
for (i in 1:(nrow(cerdas71)-1)) {
  if(cerdas71$Animal[i]!=cerdas71$Animal[i+1] & cerdas71$Ciclo[i]!="00" & is.na(cerdas71$Fecha.Parto[i])){
    
    dias1a_baja<-c(dias1a_baja,  cerdas71$Fecb[i]-cerdas71$Fecha.Cub[i])
  }
  else if(i+1==nrow(cerdas71) & is.na(cerdas71$Tipo.Parto[i])){
    dias1a_baja<-c(dias1a_baja,  cerdas71$Fecb[i+1]-cerdas71$Fecha.Cub[i+1])
  }
}
mean(dias1a_baja)
```
...
for (i in 1:(nrow(cerdas72)-1)) {
  if(cerdas72$Animal[i]!=cerdas72$Animal[i+1] & cerdas72$Tipo.Parto[i]==1){
    ...
    diasAb_baja<-c(diasAb_baja,  cerdas72$Fecb[i]-cerdas72$Fecha.Parto[i])
  }
  else if(i+1==nrow(cerdas72)& cerdas72$Tipo.Parto[i+1]==1){
    diasAb_baja<-c(diasAb_baja,  cerdas72$Fecb[i+1]-cerdas72$Fecha.Parto[i+1])
  }
}
mean(diasAb_baja)
```
...
for (i in 1:(nrow(cerdas73)-1)) {
  if(cerdas73$Animal[i]!=cerdas73$Animal[i+1] & cerdas73$Tipo.Parto[i]==0){
    ...
    diasDest_baja<-c(diasDest_baja,  cerdas73$Fecb[i]-cerdas73$FECHA.DESTETE[i])
  }
  else if(i+1==nrow(Altabaja)& cerdas73$Tipo.Parto[i+1]==0){
    diasDest_baja<-c(diasDest_baja,  cerdas73$Fecb[i+1]-cerdas73$FECHA.DESTETE[i+1])
  }
}
mean(diasDest_baja)
```
